...
class ServiceCore
{
	...
	{
        return Registry::getRequestParameter($sParamName);
	}
	...
	function readImvaConfig($sModuleName = null, $sParam = null)
	{
		if ($sModuleName and $sParam)
		{
			...
			return DatabaseProvider::getDb(true)->getone($sSqlRequest);
		}
	...
	public function log($sModuleName,$sAction,$d1 = '',$d2 = '',$d3 = '',$d4 = '',$p = ''){
        ...
        DatabaseProvider::getDb(true)->execute($sSqlRequest);
	}
	...
	{
		if ($sModuleId) {
            $oTmpModuleObj = oxNew(\OxidEsales\Eshop\Core\Module\Module::class);
    ...
    {
        $Logger = oxNew(\Imva\OxidServices\Service\Logger::class);
